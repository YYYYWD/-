...
public class ExcelUtils {
    ...
    public static void readFromExcelDemo() throws IOException {
         ...
         * 读取Excel表中的所有数据
         */
        Workbook workbook = getWeebWork("test.xlsx");
    ...
    public static List<tab_customer>  readFromExcelDemo1(String filepath) throws IOException {
        /**
         * 读取Excel表中的所有数据
         *
        ...
        list=new ArrayList<tab_customer>();
        ...
        for (int i = 0; i <= rownum; i++) {
            ...
            for (int j = row.getFirstCellNum(); j < row.getLastCellNum(); j++) {// 遍历一行中的所有列
                ...
                //System.out.print("这是第"+i+"行的第"+j+"个"+celldata + "\t");//
                if (j==0)
                    customer.setGuestName(String.valueOf(celldata));
                if (j==1)
                   customer.setGuestSex(String.valueOf(celldata));
                if (j==2)
                    customer.setGuestID(String.valueOf(celldata));
                if (j==3)
                    customer.setGuestTele(String.valueOf(celldata));
            }
    ...
    public static void writeToExcelDemo1(String filepath,List<tab_customer>mylist) throws IOException {
        /**
       ...
       // headList.add("用户名");// excel的都
        ...
        for (int i = 0; i < mylist.size(); i++)
        {
            ...
            treeMap.put("m1",String.valueOf(mylist.get(i).getGuestName()));
            treeMap.put("m2",String.valueOf( mylist.get(i).getGuestSex()));
            treeMap.put("m3",String.valueOf( mylist.get(i).getGuestID()));
            treeMap.put("m4", String.valueOf(mylist.get(i).getGuestTele()));
        ...
        writeExcel(filepath, map, wb);//往wb里面写map中内容，生成E:/test1.xlsx这个文件....
    }
}
