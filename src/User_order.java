...
public class User_order extends JPanel {
    List<tab_BOOK> list;
    ...
    Object[][] cellData = {{"", "", "", "", "", ""}};
    String[] columnNames = {"机票单号", "航班号","出发城市","目的城市","出发时间","客户姓名","座位编号"};
    String MyuserNo;
    //String []tableHeader = new String[]{"CustNo", "GuestName", "GuestSex", "GuestID", "GuestTele"};
    User_order(String User_No) {
        //this.setUndecorated(true);
        MyuserNo=new String(User_No);
    ...
    public void FindALL(String User_No) {
        String sql1 = "select * from BOOK_TABLE B ,AIR_TABLE A where A.AIRLINENO=B.AIRLINENO " +
                "AND CustNo='"+User_No+"'";
        SQL mysql = new SQL();
        try {
            ...
            while (rs.next()) {
                ...
                cust.setTicketNO(rs.getString("TicketNo"));
                cust.setAirLineNo(rs.getString("AirlineNo"));
                cust.setStart(rs.getString("StartCity"));
                cust.setEnd(rs.getString("Endcity"));
                cust.setStartTime(rs.getTimestamp("StartTime"));
                cust.setCustomerName(rs.getString("CustomerName"));
                cust.setSitNo(rs.getString("sitNo"));
        ...
        for (int i = 0; i < list.size(); i++) {
            for (int j = 0; j < columnNames.length; j++) {
                ...
                cellData[i][2] = list.get(i).getStart();
                cellData[i][3] = list.get(i).getEnd();
                cellData[i][4]=list.get(i).getStartTime();
                cellData[i][5]=list.get(i).getCustomerName();
                cellData[i][6]=list.get(i).getSitNo();
    ...
    public void BRUSH(String User_No)
    {
        this.list=new ArrayList<tab_BOOK>();
        String sql1 = "select * from BOOK_TABLE B ,AIR_TABLE A where A.AIRLINENO=B.AIRLINENO " +
                "AND CustNo='"+User_No+"'";
