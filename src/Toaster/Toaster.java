...
public class Toaster {
    ...
    public void warn1(String... messages) {
        //System.out.println("warn!!!");
        for (String s : messages) {
            toast(s, UIUtils.COLOR_INTERACTIVE);
        }
    }
    public void Mywarn(int Xlength,int Ylength ,Color myColor,String...messages)
    {
        Color givenColor=myColor;
        for (String s : messages) {
            mytoast(s, givenColor,Xlength,Ylength);
        }
    }
    ...
    private void mytoast(String message, Color bgColor,int Xlength,int Ylength) {
        ToasterBody toasterBody;

        if (toasterBodies.isEmpty()) {
            toasterBody = new ToasterBody("my",Xlength,Ylength,panelToToastOn, message, bgColor, STARTING_Y_POS);
            CURRENT_Y_OFFSET.set(STARTING_Y_POS + toasterBody.getHeightOfToast());
        } else {
            toasterBody = new ToasterBody("my",Xlength,Ylength,panelToToastOn, message, bgColor, CURRENT_Y_OFFSET.get() + SPACER_DISTANCE);
            CURRENT_Y_OFFSET.addAndGet(SPACER_DISTANCE + toasterBody.getHeightOfToast());
        }
